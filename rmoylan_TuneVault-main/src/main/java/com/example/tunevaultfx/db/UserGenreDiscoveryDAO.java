...
public final class UserGenreDiscoveryDAO {
    ...
    public void save(String username,
        ...
        String csv = serializeWeights(normalizedBoosts);
        String m = mode == null || mode.isBlank() ? "FULL" : mode.trim().toUpperCase();

        String sql = """
                INSERT INTO user_genre_discovery
                    (user_id, top_genre, second_genre, third_genre, quiz_mode, weights_boost)
                VALUES (?, ?, ?, ?, ?, ?)
    ...
    private Optional<UserGenreDiscoverySummary> loadSummaryWithoutQuizMode(int userId) throws SQLException {
        String sql = """
    ...
    private static Optional<UserGenreDiscoverySummary> mapSummaryRow(ResultSet rs, boolean readQuizMode)
            throws SQLException {
        String quiz =
                readQuizMode ? rs.getString("quiz_mode") : null;
        if (readQuizMode && rs.wasNull()) {
            quiz = null;
        }
        UserGenreDiscoverySummary s =
                new UserGenreDiscoverySummary(
    ...
    private static boolean isUnknownColumnError(SQLException e, String columnName) {
        ...
        return m != null && m.contains("Unknown column") && columnName != null && m.contains(columnName);
    }
    ...
    private static String serializeWeights(Map<String, Double> boosts) {
        ...
        return boosts.entrySet().stream()
                .filter(e -> e.getKey() != null && !e.getKey().isBlank() && e.getValue() != null && e.getValue() > 0)
