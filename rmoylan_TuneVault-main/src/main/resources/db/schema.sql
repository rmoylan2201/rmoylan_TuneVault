...
-- ---------------------------------------------------------------------------
-- Genre Discovery Quiz — question bank stored in the database
-- ---------------------------------------------------------------------------

-- quiz_question: one row per question prompt.
--   quiz_mode: QUICK (shown in both modes, first 5) or FULL (full quiz only)
--   display_order: ascending sort order; QUICK questions must have the lowest orders
CREATE TABLE IF NOT EXISTS quiz_question (
    question_id   INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    prompt        VARCHAR(512) NOT NULL,
    quiz_mode     ENUM('QUICK','FULL') NOT NULL DEFAULT 'FULL'
                  COMMENT 'QUICK = first 5 shown in both modes; FULL = full quiz only',
    display_order TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Ascending sort; 1–5 = QUICK, 6–10 = FULL',
    is_active     TINYINT(1) NOT NULL DEFAULT 1 COMMENT '0 = hidden without deleting',
    CONSTRAINT uq_quiz_question_order UNIQUE (display_order)
) ENGINE=InnoDB;

-- quiz_answer: four answers per question, each mapped to a genre name.
--   genre_name must match values in the genre table (case-insensitive compare in app).
--   weight: scoring multiplier (1 = normal, 2 = counts double).
