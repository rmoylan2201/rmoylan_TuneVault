...
/**
 * Handles shared music playback logic.
 * Plays songs, pauses songs, changes tracks, and tracks the current queue state.
 */
public class MusicPlayerService {
    ...
    public void onSongRemovedFromPlaylist(String playlistName, Song removedSong) {
        ...
        if (playlistName == null || !playlistName.equals(currentSourcePlaylistName.get())) {
            return;
        }
    ...
    public void addCurrentSongToPlaylist(String playlistName) {
        UserProfile profile = SessionManager.getCurrentUserProfile();
        if (profile == null || currentSong.get() == null) {
            return;
        }
    ...
    private void tick() {
        if (currentSong.get() == null || !playing.get()) {
            return;
        }
